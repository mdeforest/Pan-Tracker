...
- 🔄 **Collection Import + Expiry Recommendations** (PR #24, 2026-04-05): Replaces 7 generic categories with 8 Sophia-specific ones (`mascara`, `cleanser`, `serum`, `moisturizer`, `mist`, `eye_cream`, `toner`, `miscellaneous`). Sophia-format CSV importer with brand auto-split, multi-step review UI (`ImportCollectionSheet`). 4 new nullable columns on `products`: `size_weight`, `manufacture_date`, `date_in_collection`, `expiration_date`. Expiry badges on product library cards (red/amber/green), product detail page (always visible), and import preview. "Expiring Soon" sort in product library. "Expiring Soon" shelf in pan view for backlog products expiring within 90 days. Unified import entry point at `/import/csv` with client-side format detection (Sophia vs. history CSV). "Wants"/"Wish List" section in Sophia CSV routed to wishlist (finished → product library, non-finished → wishlist). History CSV shows inline row editing with server round-trip validation. Single "Import" nav entry in UserMenu + SideNav. 198 tests pass.

## Collection Import + Expiry Recommendations Deviations & Notes

- **Category replacement is a breaking change:** All existing products were migrated to `miscellaneous` by the `20260405000001_replace_product_categories.sql` migration. Seed data updated accordingly. PostgreSQL `ENUM` can't `DROP VALUE`, so migration casts to `text`, drops, recreates, casts back.
- **`expirationDate` is nullable in the schema:** `collectionRowSchema` has `expirationDate: z.string().trim().nullable()` even though the parser always computes a fallback value. This allows the API to accept rows without expiry dates for future flexibility.
- **Expiry date parsing uses `T00:00:00` suffix:** `new Date(dateStr + "T00:00:00")` is used in `ExpiryBadge` and `ExpiringSoonShelf` to parse ISO date-only strings as local midnight rather than UTC midnight.
- **Expired products appear in the expiry shelf:** The `getExpiringSoonProducts` query has no lower bound — it fetches products with `expiration_date <= today+90d`, which includes already-expired items. These show "Expired" badges on the shelf, which is intentional.
- **`EXPIRY_SHELF_DAYS = 90` constant** lives in `lib/constants.ts`.
- **Brand auto-split list** is in `lib/import/brands.ts` as `KNOWN_BRANDS_SORTED` (sorted longest-first to prevent prefix shadowing). Add brands there; no other files need updating.
- **Unified import entry point:** `/import/csv` (single page). Client reads first CSV line, calls `detectSophiaFormat(headers)` — if true, opens `ImportCollectionSheet` with the file via `initialFile` prop; otherwise proceeds with the history import flow. No server round-trip needed for detection.
- **`ImportCollectionSheet.initialFile` prop:** When provided, file is pre-loaded and category auto-detected; user stays on "file" step to confirm/change category before parsing. File input replaced by a read-only filename display.
- **Wishlist import from Sophia CSV:** Items under "Wants"/"Want"/"Wish List"/"Wishlist" section headers — `isFinished=true` rows go to `rows` (create product + empty record); others go to `wishlistRows` which are imported as wishlist items via `importCollectionRows`.
- **History CSV inline row editing:** Non-fatal row errors show "Fix" expand UI in `CsvImportClient`. Validated client-side by posting a mini 2-row CSV to `mode=preview`. Final import uses `mode=import-rows` (JSON endpoint calling `importParsedRows`) when corrections exist.
- **`ExpiryBadge alwaysShow` prop:** Added for contexts where all expiry dates should be visible regardless of proximity. Default behavior (cards) unchanged — only shows within 180 days.
- **Product detail metadata block:** Shows size/weight, date in collection, manufacture date when set; expiry badge always visible in the chip row.
- **Migration cast-first strategy:** `20260405000001` casts category column to `text` before `DROP TYPE` to avoid PostgreSQL dependency errors; uses `DROP TYPE IF EXISTS` for idempotency.
